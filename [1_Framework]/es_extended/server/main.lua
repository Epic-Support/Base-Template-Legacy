...
local loadPlayer = 'SELECT `accounts`, `job`, `job_grade`, `job2`, `job2_grade`, `position`, `inventory`, `skin`, `loadout`, `metadata`'
local loadGroup = 'SELECT `group` FROM `user_perm` WHERE `identifier` = ?'
...
local function createESXPlayer(identifier, playerId, data)
	...
	MySQL.prepare(newPlayer, parameters, function()
		-- Insérer le groupe dans la table `user_perm`
        MySQL.prepare('INSERT INTO `user_perm` (`identifier`, `group`, `steam_id`, `ip`, `nom`, `date_ajout`) VALUES (?, ?, ?, ?, ?, ?)', 
        { identifier, defaultGroup, "N/A", "0.0.0.0", "Inconnu", os.date("%Y-%m-%d %H:%M:%S") }, function()
            -- Charger les données du joueur après création
...
function loadESXPlayer(identifier, playerId, isNew)
    ...
    -- Vérification des valeurs retournées par la BDD
    if not result then
        print(("[ERROR] Impossible de charger les données de %s"):format(identifier))
        return
    end

    -- Debug: Vérification des valeurs retournées
    --print(("[DEBUG] Chargement du joueur %s - Job en BDD: %s (%s) | Job2 en BDD: %s (%s)"):format(identifier, result.job or "nil", result.job_grade or "nil", result.job2 or "nil", result.job2_grade or "nil"))
    ...
    -- **Correction: Job2 chargé comme dans SavePlayer**
    ...
    if not ESX.DoesJobExist(job2, grade2) then
        print(("[WARNING] Job2 invalide pour %s - [Job2: %s | Grade: %s]"):format(identifier, job2, grade2))
    ...
    -- Group
	local groupResult = MySQL.prepare.await('SELECT `group` FROM `user_perm` WHERE `identifier` = ?', { identifier })
	if groupResult then
		if groupResult == "superadmin" then
			userData.group = "superadmin"
			print(("[INFO] Groupe défini sur superadmin pour l'utilisateur %s"):format(identifier))
		else
			userData.group = groupResult
			print(("[INFO] Groupe défini sur : %s pour l'utilisateur %s"):format(groupResult, identifier))
		end
	else
		userData.group = 'user' -- Par défaut
		print(("[WARNING] Aucun groupe trouvé pour l'utilisateur %s. Groupe par défaut : user"):format(identifier))
	end
    ...
    --print(("[DEBUG] xPlayer créé pour %s - Job2: %s (%s)"):format(identifier, xPlayer.job2.name, xPlayer.job2.grade))
...
AddEventHandler("playerDropped", function(reason)
    ...
    if xPlayer then
        ...
        -- Gestion du second job (job2) uniquement s'il existe
        ...
        -- Sauvegarde complète du joueur (job1 + job2 + données)
...
AddEventHandler("esx:playerLoaded", function(_, xPlayer)
    ...
    -- Gestion du job secondaire (job2)
    local job2 = xPlayer.getJob2() and xPlayer.getJob2().name or nil
    if job2 and job2 ~= "unemployed" then
