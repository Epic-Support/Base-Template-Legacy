    ...
    CreateThread(function()
        ...
        ESX.GetVehicleType(vehicleModel, xPlayer.id, function(vehicleType)
            if not vehicleType then
                print(("❌ Tried to spawn invalid vehicle - ^5%s^7!"):format(model)) -- ✅ Garde seulement l'affichage

                if promise then
                    return promise:reject() -- ❌ Enlève le message d'erreur
                end
                return
            end
            ...
            while not createdVehicle or createdVehicle == 0 or NetworkGetEntityOwner(createdVehicle) == -1 do
                ...
                tries = tries + 1
                if tries > 40 then
                    print(("❌ Could not spawn vehicle - ^5%s^7!"):format(model)) -- ✅ Remplace l'erreur par un print
