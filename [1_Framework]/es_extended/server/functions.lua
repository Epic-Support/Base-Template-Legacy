...
ESX.Table = ESX.Table or {}

-- Vérifie si une table contient une valeur spécifique
function ESX.Table.Contains(table, value)
...
local function GetPlayerGroup(identifier)
    local result = MySQL.single.await("SELECT `group` FROM `user_perm` WHERE `identifier` = ?", {identifier})
    if result and result.group then
        return result.group
    end
    return nil -- Aucun groupe trouvé
end
...
-- Vérifier le groupe de l'utilisateur depuis la table user_perm
local playerGroup = xPlayer and GetPlayerGroup(xPlayer.getIdentifier())

if not playerGroup then
    error = TranslateCap("commanderror_notallowed")
else
    -- Vérification pour les groupes multiples
    if type(command.group) == "table" then
        local isAuthorized = false
        for _, group in ipairs(command.group) do
    ...
    -- Vérification pour un seul groupe
    elseif type(command.group) == "string" then
        if playerGroup ~= command.group then
            error = TranslateCap("commanderror_notallowed")
        end
    end
end
...
function Core.SavePlayer(xPlayer, cb)
    -- Mise à jour des métadonnées de santé et d'armure
    updateHealthAndArmorInMetadata(xPlayer)

    -- Vérification des valeurs du job2
    local job2Name = xPlayer.job2 and xPlayer.job2.name or "unemployed2"
    local job2Grade = xPlayer.job2 and xPlayer.job2.grade or 0

    --print(("[DEBUG] Saving %s - Job2: %s (%s)"):format(xPlayer.identifier, job2Name, job2Grade))

    -- Préparer les paramètres pour la sauvegarde
    local parameters = {
        json.encode(xPlayer.getAccounts(true)), -- Comptes bancaires
        xPlayer.job.name, -- Nom du job principal
        xPlayer.job.grade, -- Grade du job principal
        job2Name, -- Nom du second job (corrigé)
        job2Grade, -- Grade du second job (corrigé)
        json.encode(xPlayer.getCoords()), -- Position du joueur
        json.encode(xPlayer.getInventory(true)), -- Inventaire
        json.encode(xPlayer.getLoadout(true)), -- Armes équipées
        json.encode(xPlayer.getMeta()), -- Métadonnées supplémentaires
        xPlayer.identifier -- Identifiant du joueur
    }

    -- Exécution de la requête de mise à jour dans la base de données
    MySQL.prepare(
        "UPDATE `users` SET `accounts` = ?, `job` = ?, `job_grade` = ?, `job2` = ?, `job2_grade` = ?, `position` = ?, `inventory` = ?, `loadout` = ?, `metadata` = ? WHERE `identifier` = ?",
        parameters,
        function(affectedRows)
            if affectedRows == 1 then
                print(('[^2INFO^7] Saved player ^5"%s^7"'):format(xPlayer.name))
                TriggerEvent('esx:playerSaved', xPlayer.playerId, xPlayer) -- Event pour d'autres scripts
            else
                print(('[^1ERROR^7] Failed to save player ^5"%s^7"'):format(xPlayer.name))
            end
            if cb then
                cb()
            end
...
function Core.SavePlayers(cb)
    ...
    -- Préparation des paramètres pour chaque joueur
    ...
    -- Exécution de la requête de mise à jour pour tous les joueurs
    MySQL.prepare(
        "UPDATE `users` SET `accounts` = ?, `job` = ?, `job_grade` = ?, `job2` = ?, `job2_grade` = ?, `position` = ?, `inventory` = ?, `loadout` = ?, `metadata` = ? WHERE `identifier` = ?",
        parameters,
        function(results)
