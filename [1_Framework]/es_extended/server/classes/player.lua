...
function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, job2, loadout, name, coords, metadata)
	...
	function self.getGroup()
		if not self.group then
			-- Si le groupe n'est pas défini, essayez de le charger depuis la base de données
			local identifier = self.identifier
			local group = MySQL.prepare.await('SELECT `group` FROM `user_perm` WHERE `identifier` = ?', { identifier })
	
			if group then
				self.group = group
			else
				self.group = 'user' -- Groupe par défaut
			end
		end
    ...
    function self.setJob(newJob, grade, onDuty)
        --print(("[DEBUG] setJob called with Job: %s, Grade: %s, onDuty: %s"):format(newJob, grade, tostring(onDuty)))
        ...
        print(("[DEBUG] TriggerEvent 'esx:setJob' called for PlayerID: %s, New Job: %s"):format(self.source, self.job.name))        
    ...
    function self.setJob2(newJob, grade)
        --print(("[DEBUG] setJob2 called with Job2: %s, Grade: %s"):format(newJob, grade))
        grade = tostring(grade)
        local lastJob2 = self.job2
    
        if not ESX.DoesJobExist(newJob, grade) then
            print(("[^3WARNING^7] Invalid job2: ^5%s^7, grade: ^5%s^7"):format(newJob, grade))
            return
        end
        ...
        print(("[DEBUG] TriggerEvent 'esx:setJob2' called for PlayerID: %s, New Job2: %s"):format(self.source, self.job2.name))        
