...
function ESX.ShowNotification(message, type, length)
    if GetResourceState('okokNotify') ~= 'missing' then
        if type == 'error' then
            exports['okokNotify']:Alert("Error", message, 3000, 'error')
        elseif type == 'inform' then
            exports['okokNotify']:Alert("Info", message, 3000, 'info')
        elseif type == 'success' then
            exports['okokNotify']:Alert("Success", message, 3000, 'success')
        elseif type == 'warning' then
            exports['okokNotify']:Alert("Warning", message, 3000, 'warning')
        else
            exports['okokNotify']:Alert("Info", message, 3000, 'info')
        end
    else
...
function ESX.ShowAdvancedNotification(sender, subject, msg, textureDict, iconType, flash, saveToBrief, hudColorIndex)
    if GetResourceState('okokNotify') == 'started' then
        local title = sender .. " - " .. subject
        exports['okokNotify']:Alert(title, msg, 3000, 'info') -- Utilise une notification simple
    else
        -- Fallback si okokNotify n'est pas disponible
...
function ESX.ShowHelpNotification(msg, thisFrame, beep, duration)
        -- Fallback aux notifications natives si okokNotify n'est pas disponible
